<template>
  <div  class="content certify content_certify">  
        ...
        <div class="fake_original_block" v-for="fake in items.sections" :key="fake.id">
            <figure class="cd-image-container">
                <div class="cd-image">
                <img :src="fake.origin" alt="Original Image">
                <range-slider class="cd-handle"     min="0"
                max="100"
                step="0.1"
                v-model="sliderValue">></range-slider>
                </div>
                <span class="cd-image-label" v-if="sliderValue != 100">Оригинал</span>

                <div class="cd-resize-img" :style="{width: sliderValue + '%'}"> <!-- the resizable image on top -->
...
export default Vue.extend({
    ...
    mounted(){
        this.$axios.get('https://cantra.a-lux.dev/api/fake')
            .then(res=>{
                this.items = res.data
...
.range-slider-knob{
    ...
    box-shadow: 0 0 0 6px rgba(0, 0, 0, 0.2), 0 0 10px rgba(0, 0, 0, 0.6), inset 0 1px 0 rgba(255, 255, 255, 0.3)  !important;
